          ...
          <div class="flex flex-col gap-4 py-4">
            <template v-for="field in currentStructure.checkbox_group" :key="field.name">
              ...
              <div v-else-if="field.component === 'checkbox'" class="flex items-start gap-3">
                <input v-model="formData[field.name]" type="checkbox" :id="'cvs_' + field.name" class="mt-1 w-5 h-5 rounded border-white/30 bg-white/10 text-yellow-500 focus:ring-yellow-500" :required="field.required" />
                <label :for="'cvs_' + field.name" class="text-white text-xs leading-relaxed select-none" v-html="field.label"></label>
              </div>
            </template>
          </div>
